...
2. If the request states no intent — who it's for, what the output enables — ask
   for that first; the "why" reshapes every downstream decision.
3. Identify the decision branches that actually change what gets built: scope, data
   shapes, UX, failure behavior, integration points. Ignore branches that don't
   change the outcome.
4. Ask ONE question at a time. With every question, state your recommended answer
   and why — the user should mostly be confirming or redirecting, not designing
   from scratch.
5. Challenge vague or contradictory answers; follow up until each decision is
