...
decide. If the work produced analysis findings, hand them to the `insights` skill;
if it was substantial enough that the user may have lost the thread, offer
`debrief`. In a scaffolded project, record a WORKLOG entry (pointers only). Tests and type checks verify CODE correctness, not FEATURE correctness —
